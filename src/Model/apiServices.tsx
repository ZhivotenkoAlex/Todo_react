...
import { IApiData, IApiFetch, IPost, IPostData } from '../types/generalTypes'
...
export default class apiServices extends Component {
  ...
  getItems = async (data: string): Promise<Error | []> =>
    fetch(`${this.props}/api/todo?Authorization=Bearer ${data}`)
      .then(res => res.clone().json())
      .catch(e => console.log(e))

  getUser = async (data: string): Promise<IUser | void> =>
    fetch(`${this.props}/api/user?item=${data}`)
      .then(res => res.clone().json())
  ...
  post = async (endPoint: string, data: IPostData): Promise<void | IPost> =>
